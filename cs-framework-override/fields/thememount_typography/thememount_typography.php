 ...
 *
 */
class CSFramework_Option_thememount_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    $google_json   = tm_cs_get_google_fonts();
    ...
    if( is_object( $google_json ) ) {
	  ...
	  echo '<small>'.esc_html__( 'Font Family', 'fixology' ).'</small><br>';
      ...
      echo '<optgroup label="'. esc_html__( 'Web Safe Fonts', 'fixology' ) .'">';
      ...
      echo '</optgroup>';
      echo '<optgroup label="'. esc_html__( 'Google Fonts', 'fixology' ) .'">';
	   ...
	   */
		if( ! empty( $is_backup_family ) ) {
			...
			echo '<small>'.esc_html__( 'Backup Font Family', 'fixology' ).'</small><br>';
      ...
      if( ! empty( $is_variant ) ) {
		...
		echo '<small>'.esc_html__( 'Font Variants', 'fixology' ).'</small><br>';
		...
		if( ! empty( $is_text_transform ) ) {
			...
			echo '<small>'.esc_html__( 'Text Transform', 'fixology' ).'</small><br>';
	   ...
	   echo '<div class="clear clr"></div>  </div> <!-- .cs-tm-typography-row2 -->';
		...
		if( ! empty( $is_font_size ) ) {
			...
			echo '<small>'.esc_html__( 'Font Size', 'fixology' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-font-size mini" min="1" max="200" step="1" title="'.esc_html__( 'Font Size', 'fixology' ).'" name="'. $this->element_name( '[font-size]' ) .'" value="'.$font_size_value.'">
		...
		if( ! empty( $is_line_height ) ) {
			...
			echo '<small>'.esc_html__( 'Line Height', 'fixology' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="1" max="200" step="1" title="'.esc_html__( 'Line Height', 'fixology' ).'" name="'. $this->element_name( '[line-height]' ) .'" value="'.$line_height_value.'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		...
		if( ! empty( $is_letter_spacing ) ) {
			echo '<div class="tm-fancy-input-w">';
			echo '<label class="cs-typography-line-height">';
			echo '<small>'.esc_html__( 'Letter Spacing', 'fixology' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="0" max="20" step="0.1" title="'.esc_html__( 'Letter Spacing', 'fixology' ).'" name="'. $this->element_name( '[letter-spacing]' ) .'" value="'.$letter_spacing_value.'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		...
		if( ! empty( $is_color ) ) {
			...
			echo '<small>'.esc_html__( 'Font Color', 'fixology' ).'</small><br>';
			echo '<input type="text" name="'. $this->element_name( '[color]' ) .'" value="'. $color_value .'"'. $this->element_class( 'cs-field-color-picker' ) . '/>';
			echo '</label>';
			echo '</div>';
		}
      ...
      if( ! empty( $is_all_varients ) ) {
        ...
        echo '<label class="cs-typography-font-Size">';
		echo '<div class="tm-typo-all-varient-input"><input type="checkbox" name="'. $this->element_name( '[all-varients]' ) .'" value"on" '. checked( 'on', $all_varients_value, false ) .' /></div>';
		echo '<div class="tm-typo-load-all-varient-text">'. esc_html__('Load all variations of the selected font (Like bold, italic etc.)','fixology') .'<br><small>'. esc_html__('Loading all font variations can slow down (a little) your webpage.','fixology').'</small></div>';
		echo '<div class="clear clr"></div>';
        echo '</label>';
		echo '</div>';
      }
      ...
      echo esc_html__( 'Error! Can not load json file.', 'fixology' );
...
function tm_cs_get_google_fonts() {
	...
	if( ! empty( $tm_cs_google_fonts ) ) {
		...
		ob_start();
		cs_locate_template( 'fields/thememount_typography/google-fonts.json' );
		$json = ob_get_clean();
