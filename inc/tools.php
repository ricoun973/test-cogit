...
if( !function_exists('thememount_logo') ){
function thememount_logo(){
	$logotype       = thememount_get_option('logotype');
	$logoimg        = thememount_get_option('logoimg');
	$logoimg_sticky = thememount_get_option('logoimg_sticky');
	...
	if( $logotype=='image' ){
		if( isset($logoimg) && is_array($logoimg) ){
			...
			if( isset($logoimg['full-url']) && trim($logoimg['full-url'])!='' ){
				...
				$return .= '<img class="thememount-logo-img standardlogo" alt="' . get_bloginfo( 'name' ) . '" src="'.$logoimg['full-url'].'">';
			
			} else if( isset($logoimg['thumb-url']) && trim($logoimg['thumb-url'])!='' ){
				$image = $logoimg['thumb-url'];
				$return .= '<img class="thememount-logo-img standardlogo" alt="' . get_bloginfo( 'name' ) . '" src="'.$logoimg['thumb-url'].'">';

			} else if( isset($logoimg['id']) && trim($logoimg['id'])!='' ){
				$image   = wp_get_attachment_image_src( $logoimg['id'], 'full' );
				$return .= '<img class="thememount-logo-img standardlogo" alt="' . get_bloginfo( 'name' ) . '" src="'.$image[0].'" width="'.$image[1].'" height="'.$image[2].'">';
			...
			if( isset($logoimg_sticky) && is_array($logoimg_sticky) ){
				...
				if( isset($logoimg_sticky['full-url']) && trim($logoimg_sticky['full-url'])!='' ){
					...
					$return .= '<img class="thememount-logo-img stickylogo" alt="' . get_bloginfo( 'name' ) . '" src="'.$logoimg_sticky['full-url'].'">';
				
				} else if( isset($logoimg_sticky['thumb-url']) && trim($logoimg_sticky['thumb-url'])!='' ){
					$sticky_image   = $logoimg_sticky['thumb-url'];
					$return .= '<img class="thememount-logo-img stickylogo" alt="' . get_bloginfo( 'name' ) . '" src="'.$logoimg_sticky['thumb-url'].'">';
				
				} else if( isset($logoimg_sticky['id']) && trim($logoimg_sticky['id'])!='' ){
					$sticky_image   = wp_get_attachment_image_src( $logoimg_sticky['id'], 'full' );
					$return .= '<img class="thememount-logo-img stickylogo" alt="' . get_bloginfo( 'name' ) . '" src="'.$sticky_image[0].'" width="'.$sticky_image[1].'" height="'.$image[2].'">';
		...
		$return = thememount_get_option('logotext');
	}
	$return .= '</span>';
...
function thememount_social_share_links( $post_type='portfolio' ){
	...
	if( !empty($post_type) ){
		...
		${ $post_type.'_social_share_services_title' } = thememount_get_option( $post_type.'_social_share_services_title' );
		
		
		$return = '';
		
		
		// Text before social share icons
		if( !empty( ${ $post_type.'_social_share_services_title' } ) ){
			$return .= '<li class="tm-social-share-titletext">' . esc_html( ${ $post_type.'_social_share_services_title' } ) . '</li>';
		}
 ...
 */
if( !function_exists('tm_shared_social_list') ){
function tm_shared_social_list(){
	/**
 ...
 *  List of Social services that used for Social Links section
 */
if( !function_exists('thememount_a_color') ){
function thememount_a_color(){
	$skincolor  = thememount_get_option('skincolor');
	$link_color = thememount_get_option('link-color');
	...
	if( $link_color=='darkhover' ){
		...
		$normal_color = thememount_get_option('link-color-regular');
		$hover_color  = thememount_get_option('link-color-hover');
 ...
 */
if( !function_exists('thememount_vc_get_bg_css_only') ){
function thememount_vc_get_bg_css_only($css, $nobg='') {
 ...
 */
if( !function_exists('thememount_pagination') ){
function thememount_pagination( $wp_query_data=false ){
	...
	$return  = '';
	$return .= thememount_wp_kses('<div class="clearfix"></div>');
	...
	$paginateLinks = paginate_links( array(
		...
		'type'      => 'array',
		'prev_text' => '<i class="tm-fixology-icon-arrow-left"></i> <span class="tm-hide tm-pagination-text tm-pagination-text-prev">' . esc_html__( 'Previous page', 'fixology' ) . '</span>',
		'next_text' => '<span class="tm-hide tm-pagination-text tm-pagination-text-next">' . esc_html__( 'Next page', 'fixology' ) . '</span> <i class="tm-fixology-icon-arrow-right"></i>',
	) );
	
	
	if( $paginateLinks!=NULL ){
		$big = 999999999; // need an unlikely integer
		$return .= '<div class="thememount-pagination">';
		...
		$return .= '</div><!-- .thememount-pagination -->';
	}
 ...
 */
if( !function_exists('thememount_get_option') ){
function thememount_get_option( $option, $inner_option='' ){
	
	global $fixology_theme_options;
	if( !is_array($fixology_theme_options) ){
		
		if( is_multisite() ){
			$fixology_theme_options = get_site_option('fixology_theme_options');
		} else {
	...
	if( isset($fixology_theme_options[$option]) ){
		$return = $fixology_theme_options[$option];
	} else {
 ...
 */
if( !function_exists('thememount_get_all_registered_sidebars') ){
function thememount_get_all_registered_sidebars(){
	global $wp_registered_sidebars;
	$return = array( '' => esc_html__('Default', 'fixology') );
	...
	if( is_array($optionslist) && count($optionslist)>0 ){
		foreach( $optionslist as $options ){
			...
			if( $options['param_name']!='content' ){
				...
				if( !empty($options['type']) && $options['type']=='thememount_iconpicker' ){
					$std = $options['value'];
				}
...
function tm_vc_element_heading( $allVar ){
	...
	if( !empty($allVar['h2']) ) {
		$return .= '<div class="thememount-box-heading-wrapper tm-element-align-'.$allVar['txt_align'].'">';
...
if( !function_exists('thememount_hex2rgb') ){
function thememount_hex2rgb($hex) {
	$hex = str_replace("#", "", $hex);
 ...
 */
if( !function_exists('thememount_get_widgets_count') ){
 ...
 *  Widget count class
 */
if( !function_exists('thememount_class_for_widgets_count') ){
...
/**
 * Custom template tags for Fixology
 *
...
if ( ! function_exists( 'fixology_comment_nav' ) ) :
/**
 * Display navigation to next/previous comments when applicable.
 *
 * @since Fixology 1.0
 */
function fixology_comment_nav() {
	// Are there comments to navigate through?
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
	?>
	<nav class="navigation comment-navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'fixology' ); ?></h2>
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'fixology' ) ) ) :
					printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;

				if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'fixology' ) ) ) :
...
if ( ! function_exists( 'fixology_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 *
 * @since Fixology 1.0
 */
function fixology_entry_meta( $metafor="blogbox" ) {
	...
	$return       = '';
	$social_share = '';
	$metalist     = thememount_get_option( $metafor . '_meta_list' );
	$date_format  = thememount_get_option( $metafor . '_meta_dateformat' );
	$cat_link     = thememount_get_option( $metafor . '_meta_catlink' );
	$tag_link     = thememount_get_option( $metafor . '_meta_taglink' );
	$author_link  = thememount_get_option( $metafor . '_meta_authorlink' );
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line posted-on"><i class="tm-fixology-icon-clock"></i> <span class="screen-reader-text tm-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
							esc_attr_x( 'Posted on', 'Used before publish date.', 'fixology' ),
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="tm-meta-line byline"><i class="tm-fixology-icon-user"></i> <span class="author vcard"><span class="screen-reader-text tm-hide">%1$s </span>%2$s</span></span>',
							esc_attr_x( 'Author', 'Used before post author name.', 'fixology' ),
							$author
						);
					}
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="tm-meta-line comments-link"><i class="tm-fixology-icon-comments-smiley"></i> ';
						ob_start();
						comments_popup_link( esc_html__( 'Leave a comment', 'fixology' ) );
				...
				case 'cat':
					...
					if ( !empty($categories_list) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line cat-links"><i class="tm-fixology-icon-category"></i> <span class="screen-reader-text tm-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Categories', 'Used before category names.', 'fixology' ),
							$categories_list
						);
					}
				...
				case 'tag':
					$tags_list = get_the_tag_list( '', _x( ', ', 'Used between list items, there is a space after the comma.', 'fixology' ) );
					if ( !empty($tags_list) ) {
						if( $tag_link!=true ){
							$tags_list = strip_tags($tags_list);
						}
						$return .= sprintf( '<span class="tm-meta-line tags-links"><i class="tm-fixology-icon-tag"></i> <span class="screen-reader-text tm-hide" tm-hide>%1$s </span>%2$s</span>',
							esc_attr_x( 'Tags', 'Used before tag names.', 'fixology' ),
							$tags_list
						);
					}
	...
	// Social share
	if( $metafor != "blogbox" ){
		$social_share = thememount_social_share_box('post');
	}
 ...
 */
if( ! function_exists('fixology_categorized_blog') ){
function fixology_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'fixology_categories' ) ) ) {
		...
		set_transient( 'fixology_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so fixology_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so fixology_categorized_blog should return false.
		return false;
	}
}
}
 ...
 */
if( !function_exists('tm_reset_tgm_infobox') ){
 ...
 *  CSS Minifier
 */
if( !function_exists('thememount_minify_css') ){
function thememount_minify_css( $css ){
	if( !empty($css) ){
		// Remove comments
 ...
 */
if( !function_exists('tm_get_options_type') ){
function tm_get_options_type( $type='thememount_background' ){
	$return = array();
	include( get_template_directory() .'/cs-framework-override/config/framework-options.php' );
 ...
 */
if( !function_exists('thememount_get_background_css') ){
function thememount_get_background_css( $element_array, $values, $exclude=array() ){
	...
	if( !empty($element_array['color_dropdown_id']) ){
		$dropdown_color = thememount_get_option($element_array['color_dropdown_id']);
	}
 ...
 *  The properties that can be set, are: background-color, background-image, background-position, background-size, background-repeat, background-origin, background-clip, and background-attachment.
 */
if( !function_exists('thememount_get_all_background_css') ){
function thememount_get_all_background_css(){
	$return = array();
	
	// Getting all "thememount_background" options 
	$element_ids = tm_get_options_type('thememount_background');
	
	
	foreach( $element_ids as $element_id=>$optionlist ){
		$selector_class = $optionlist['output'];
		// modify thissss
		$element_id_val = thememount_get_option($element_id);
		$return[]       = thememount_get_background_css( $optionlist, $element_id_val );
	}
 ...
 */
if( !function_exists('thememount_get_all_font_css') ){
function thememount_get_all_font_css(){
	$return = array();
	
	// Getting all "thememount_background" options 
	$element_ids = tm_get_options_type('thememount_typography');
	
	
	foreach( $element_ids as $element_id=>$optionlist ){
		$selector_class = $optionlist['output'];
		$element_id_val = thememount_get_option($element_id);
		$return[] = thememount_get_font_css( $selector_class, $element_id_val );
	}
 ...
 */
if( !function_exists('thememount_get_font_css') ){
function thememount_get_font_css( $selector, $values, $important=false ){
	$return = array();
	$family = '';
 ...
 */
if( !function_exists('tm_check_dark_color') ){
 ...
 */
if( !function_exists('tm_mmmenu_theme_setup') ){
function tm_mmmenu_theme_setup(){
 ...
 */
if( !function_exists('thememount_body_start_code') ){
function thememount_body_start_code(){
	$return               = '';
	$page_loader          = '';
	$customhtml_bodystart = trim(thememount_get_option('customhtml_bodystart'));
 ...
 *  Custom code - Body start code
 */
if( !function_exists('thememount_get_page_loader_css') ){
function thememount_get_page_loader_css(){
	$return             = '';
	$loaderimg          = thememount_get_option('loaderimg');
	$loaderimage_custom = thememount_get_option('loaderimage_custom');
 ...
 */
if ( !function_exists( 'thememount_blogbox_footer' ) ){
function thememount_blogbox_footer(){
 ...
 *
 * Create your own thememount_blogbox_comment_count() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 */
if ( !function_exists( 'thememount_blogbox_comment_count' ) ){
function thememount_blogbox_comment_count( $echo = false ) {
	$return = '';
	if( comments_open() ){
		$comments = wp_count_comments( get_the_ID() );
		$comments = $comments->approved; //Get Total Comments
		$return  .= '<div class="thememount-blogbox-comment"><i class="tm-fixology-icon-comment"></i> '. $comments .'</div>';
	}
	return $return;
}
}
 ...
 */
if ( !function_exists( 'thememount_featured_image' ) ){
function thememount_featured_image($size='full'){
	$return = '';
 ...
 */
if ( !function_exists( 'thememount_team_member_single_meta' ) ){
function thememount_team_member_single_meta( $type='position', $post_id='' ) {
	$return = '';
	if( empty($post_id) ){
		$post_id = get_the_ID();
	}
	...
	if( !empty($type) ){
		...
		if( in_array( $type, $types ) ){
			$meta_data = get_post_meta( $post_id, 'thememount_team_member_details', true );
		...
		if( !empty($return) ){
			...
			switch( $type ){
				...
				case 'phone':
						$return = '<div class="tm-detail-icon"><i class="tm-fixology-icon-phone tm-skincolor"></i></div><div class="tm-exdetails"><div class="tm-team-list-title">'. esc_attr__('Phone','fixology') .'</div>
							<div class="tm-team-list-value"><a href="tel:' . $return . '">' . $return . '</a></div></div>';
					break;
					
				case 'email':
					$return = '<div class="tm-detail-icon"><i class="tm-fixology-icon-mail tm-skincolor"></i></div><div class="tm-exdetails"><div class="tm-team-list-title">'. esc_attr__('Email','fixology') .'</div>
						<div class="tm-team-list-value"><a href="mailto:' . $return . '">' . $return . '</a></div></div>';
					break;
				...
				case 'website':
					...
					$return = '<div class="tm-detail-icon"><i class="tm-fixology-icon-world tm-skincolor"></i></div><div class="tm-exdetails"><div class="tm-team-list-title">'. esc_attr__('Website','fixology') .'</div>
						<div class="tm-team-list-value"><a target="_blank" href="' . esc_url($return_link) . '">' . $return . '</a></div></div>';
					break;
 ...
 */
if ( !function_exists( 'thememount_team_member_meta_details' ) ){
function thememount_team_member_meta_details( $post_id='' ){
	$return  = '';
	$phone   = thememount_wp_kses( thememount_team_member_single_meta( 'phone' ) );
	$email   = thememount_wp_kses( thememount_team_member_single_meta( 'email' ) );
	$website = thememount_wp_kses( thememount_team_member_single_meta( 'website' ) ); 
 ...
 */
if ( !function_exists( 'thememount_team_member_extra_details' ) ){
function thememount_team_member_extra_details( $post_id='' ){
	$return                   = '';
	$team_extra_details_lines = thememount_get_option('team_extra_details_lines');
	...
	if( !empty($team_extra_details_lines) && is_array($team_extra_details_lines) && count($team_extra_details_lines) > 0 ){
		...
		$post_meta = get_post_meta( $post_id, 'thememount_team_member_details', true );
		
		foreach( $team_extra_details_lines as $key=>$val ){
			if( !empty($post_meta['tm_team_info']['team_extra_details_line_'.$key]) ){
				$icon  = thememount_create_icon_from_data( $val['team_extra_details_line_icon'], true );
				$title = (!empty($val['team_extra_details_line_title'])) ? sprintf( esc_html__('%s ', 'fixology'), $val['team_extra_details_line_title'] ) : '' ;
				$value =  $post_meta['tm_team_info']['team_extra_details_line_'.$key];
				$return .= '<li class="tm-team-details-line tm-team-extra-details-line-' . $key . '">
					<div class="tm-detail-icon tm-skincolor">' . $icon . '</div><div class="tm-exdetails"><div class="tm-team-list-title">' . $title . '</div>
					<div class="tm-team-list-value">' . $value . '</div></div>
				</li>';
			}
		} // foreach
 ...
 *  Icon from array
 */
if ( !function_exists( 'thememount_create_icon_from_data' ) ){
function thememount_create_icon_from_data( $data, $i_tag_only=false ){
 ...
 *  Single Team member content
 */
if ( !function_exists( 'thememount_team_member_content' ) ){
function thememount_team_member_content( $post_id='' ){
	$return = '';
 ...
 */
if ( !function_exists( 'thememount_team_member_single_category_list' ) ){
function thememount_team_member_single_category_list( $post_id='' ){
	$return = '';
 ...
 *  Single Team member extra details (list items)
 */
if ( !function_exists( 'thememount_team_member_single_excerpt' ) ){
function thememount_team_member_single_excerpt(){
	$return  = '';
...
/******************************************************************/
/* ----------------------- Portfolio box ------------------------- */
if ( !function_exists( 'thememount_portfolio_next_prev_btn' ) ){
function thememount_portfolio_next_prev_btn() {
	$return = '';
	
	if( is_singular('tm_portfolio') ){
		
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav tm-hide" aria-hidden="true">' . esc_html__( 'Next', 'fixology' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . esc_html__( 'Next post:', 'fixology' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
			'prev_text' => '<span class="meta-nav tm-hide" aria-hidden="true">' . esc_html__( 'Previous', 'fixology' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . esc_html__( 'Previous post:', 'fixology' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
		) );
...
if ( !function_exists( 'thememount_portfolio_category' ) ){
function thememount_portfolio_category( $link=true ) {
...
if ( !function_exists( 'thememount_portfolio_single_view' ) ){
function thememount_portfolio_single_view() {
	$view                = '';
	$portfolio_viewstyle = thememount_get_option('portfolio_viewstyle');
	...
	$single_viewstyle = get_post_meta( get_the_ID(), 'thememount_portfolio_view', true );
...
function thememount_portfolio_detailsbox(){
	$return                    = '';
	$portfolio_project_details = thememount_get_option('portfolio_project_details');
	$pf_details_line           = thememount_get_option('pf_details_line');
	
	// Box title
	$box_title = '';
	if( !empty($portfolio_project_details) ){
		$box_title = esc_html( $portfolio_project_details );
	}
	if( !empty($box_title) ){
		$box_title = '<h2 class="thememount-pf-detailbox-title">'.$box_title.'</h2>';
	}
	...
	$page_values = get_post_meta( get_the_ID(), 'thememount_portfolio_list_data', true );
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			$row_title = '';
			$row_value = '';
			$icon      = thememount_create_icon_from_data( $val['pf_details_line_icon'], true );
			
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( esc_html__('%s ', 'fixology'), $val['pf_details_line_title'] ); }
...
/**
 *  Blogbox class : thememount_portfoliobox_class()
 */
if ( !function_exists( 'thememount_portfoliobox_class' ) ){
function thememount_portfoliobox_class(){
	$return = '';
	return $return;
}
}
...
if ( !function_exists( 'thememount_portfoliobox_media_link' ) ){
function thememount_portfoliobox_media_link( $default_icon='tm-fixology-icon-search', $video_icon='tm-fixology-icon-video', $audio_icon='tm-fixology-icon-music-alt', $slider_icon='tm-fixology-icon-gallery-1' ){

	$icon_code='<i class="tm-fixology-icon-search"></i>';
	...
	$portfolio_featured = get_post_meta( get_the_ID(), 'thememount_portfolio_featured' , true );
	
	// default output
	$return = '<a class="tm_prettyphoto" title="' . get_the_title() . '" href="' . esc_url(thememount_portfolio_single_image_path()) . '">' . $icon_code . '</a>';
	...
	if( !empty($portfolio_featured['featuredtype']) ){
		...
		switch( $portfolio_featured['featuredtype'] ){
			...
			case 'slider':
				...
				$return = '<a title="' . get_the_title() . '" class="thememount-open-gallery" data-id="' . get_the_ID() . '" href="#thememount-embed-code-' . get_the_ID() . '">' . $icon_code . '</a>';
			...
			case 'audioembed':
				...
				$return = '<a title="' . get_the_title() . '" class="tm_prettyphoto" href="#thememount-embed-code-' . get_the_ID() . '">' . $icon_code . '</a>';
				if( !empty($portfolio_featured['audio_code']) ){
					$return .= '<div class="tm-hide" id="thememount-embed-code-' . get_the_ID() . '">' . $portfolio_featured['audio_code'] . '</div>';
				}
				break;
...
if ( !function_exists( 'thememount_gallerystyle_portfoliobox_media_link' ) ){
function thememount_gallerystyle_portfoliobox_media_link( $default_icon='tm-fixology-icon-search', $video_icon='tm-fixology-icon-video', $audio_icon='tm-fixology-icon-music-alt', $slider_icon='tm-fixology-icon-gallery-1' ){
	...
	// default output
	$return = '<a class="tm_prettyphoto" data-gal="prettyPhoto[gal]" title="' . get_the_title() . '" href="' . esc_url(thememount_portfolio_single_image_path()) . '">' . $icon_code . '</a>';
...
if ( !function_exists( 'thememount_portfoliobox_footer' ) ){
function thememount_portfoliobox_footer(){
	$return = '';
	return $return;
}
}

if ( !function_exists( 'thememount_portfoliobox_likes' ) ){
function thememount_portfoliobox_likes(){
	$return = '';	
	...
	$likeIconClass   = ( isset($_COOKIE["thememount_likes_".get_the_ID()]) ) ? 'tmicon-fa-heart' : 'tmicon-fa-heart-o' ;
		...
		$like = '<div class="thememount-portfolio-likes-wrapper">
...
if ( !function_exists( 'thememount_get_meta' ) ){
function thememount_get_meta( $meta_group='', $meta_id='' , $meta_sub_id='' ){
 $return = '';
...
function thememount_featured_gallery_slider(){
	$return = '';
	return $return;
}
}
 ...
 *  Social share box
 */
if ( !function_exists( 'thememount_social_share_box' ) ){
function thememount_social_share_box($post_type='portfolio'){
	
	$return       = '';
	$social_links = '';
	$button_html  = '';
	$portfolio_single_top_btn_title = thememount_get_option('portfolio_single_top_btn_title');
	$portfolio_single_top_btn_link  = thememount_get_option('portfolio_single_top_btn_link');
	$portfolio_social_share_title   = thememount_get_option('portfolio_social_share_title');
	
	
	// preparing social links
	switch($post_type){
		case 'portfolio':
			if( thememount_get_option('portfolio_show_social_share')==true ){
				$social_links = thememount_social_share_links('portfolio');
			}
			break;
		case 'post':
			$social_links = thememount_social_share_links('post');
			break;
	}
	
	
	// preparing button
	if( !empty($portfolio_single_top_btn_title) && !empty($portfolio_single_top_btn_title) && shortcode_exists('tm-btn') ){
 ...
 *  Post thumbnail. This will echo post thumbnail according to port format like video, audio etc.
 */
if ( !function_exists( 'thememount_portfolio_featured_media' ) ){
function thememount_portfolio_featured_media( $post_id='', $size='full' ){
	...
	// get post meta
	$post_meta = get_post_meta( $post_id, 'thememount_portfolio_featured', true );
	...
	if( !empty($post_meta['featuredtype']) ){
		...
		switch($post_meta['featuredtype']){
			case 'image':
				$return .= thememount_get_featured_media();
 ...
 */
if ( !function_exists( 'thememount_portfolio_description' ) ){
function thememount_portfolio_description( $post_id='' ){
	$return = '';
 ...
 *  Related Portfolio
 */
if ( !function_exists( 'thememount_portfolio_related' ) ){
function thememount_portfolio_related(){
	$portfolio_show_related   = thememount_get_option('portfolio_show_related');
	$portfolio_related_column = thememount_get_option('portfolio_related_column');
	$portfolio_related_show   = thememount_get_option('portfolio_related_show');
	$portfolio_related_view   = thememount_get_option('portfolio_related_view');
	$portfolio_related_title  = thememount_get_option('portfolio_related_title');
	...
	if( $portfolio_show_related===true ){
		...
		$cpt           = 'portfolio'; // This will be used in thememount_get_boxes() automatically.
		...
		if ( $posts->have_posts() ) {
			$return .= '<div class="tm-pf-single-related-wrapper">
				' . $related_title . '
				' . thememount_get_boxes( 'portfolio', get_defined_vars() ) . '
			</div>';
		}
 ...
 */
if ( !function_exists( 'thememount_post_class' ) ){
function thememount_post_class( $class_list='' ){
	$return = '';
	$classes = array();
	
	// If no featured content found
	if( thememount_get_featured_media()=='' ){
 ...
 */
if ( !function_exists( 'thememount_postlayout_class' ) ){
function thememount_postlayout_class(){
	$return = '';
	$classes = array();
...
if ( !function_exists( 'thememount_get_post_format_icon' ) ){
function thememount_get_post_format_icon( $post_id='' ){
	$return = '';
	...
	if( $post_type=='post' ){
		...
		$return = '<i class="tm-fixology-icon-' . $icon_class . '"></i>';
 ...
 */
if ( !function_exists( 'thememount_get_featured_media' ) ){
function thememount_get_featured_media( $post_id='', $size='full', $imgonly=false ){
	...
	if( !empty($post_id) ){
		...
		if( $post_type=='post' ){
			...
			$slide_images  =  get_post_meta( $post_id, '_thememount_metabox_gallery', true) ;
		...
		switch( $featured_type ){
			...
			default:
				if ( has_post_thumbnail() ) {					
					...
					global $tm_blog_imglink;
					if($tm_blog_imglink == 'yes') {
			...
			case 'video':
				if( $video_code!='' ){
					$return .= thememount_oembed_get($video_code);
				}
				break;
			...
			case 'audioembed':
				if( $audio_code!='' ){
					$return .= thememount_oembed_get($audio_code);
			...
			case 'gallery':
			case 'slider':
				$return .= thememount_create_slider($slide_images, $size);
				break;
				
			case 'quote':
				$return .= thememount_featured_quote();
				break;
				
			case 'link':
				$return .= thememount_featured_link();
				break;
		}
 ...
 *  Post Format - Link
 */
if ( !function_exists( 'thememount_featured_link' ) ){
function thememount_featured_link($code=''){
 ...
 *  Post Format - Quote
 */
if ( !function_exists( 'thememount_featured_quote' ) ){
function thememount_featured_quote($code=''){
...
if ( !function_exists( 'thememount_oembed_get' ) ){
function thememount_oembed_get($code=''){
	$return = '';
...
if ( !function_exists( 'thememount_create_slider' ) ){
function thememount_create_slider($images='', $size='full'){
	$return = '';
 ...
 * get Boxes for CPT
 */
if( !function_exists('thememount_get_boxes') ){
function thememount_get_boxes( $cpt='blog', $vars=array() ){
	global $tm_blog_imglink;
	...
	$allword  = (!empty($vars['allword'])) ? $vars['allword'] : esc_attr__('All', 'fixology');
	...
	$tm_blog_imglink	= (!empty($vars['postlink_onimage']) && $vars['postlink_onimage']=='true' )   ? 'yes' : 'no' ;
	...
	if( !empty($boxview) && $boxview == 'slickview' && $cpt == 'testimonial' ){
		$template 			= 'slickview';
		$column				= 'one';
		$startwrapper 		= thememount_column_div('start', $column ).'<div class="testimonial_wrapper">';
		$closewrapper 		= '</div>'.thememount_column_div('end', $column );
	...
	} elseif(!empty($boxview) && $boxview == 'slickview-leftimg' && $cpt == 'testimonial') {
		$template 			= 'slickview';
		$column				= 'one';
		$startwrapper 		= thememount_column_div('start', $column ).'<div class="testimonial_wrapper">';
		$closewrapper 		= '</div>'.thememount_column_div('end', $column );
		...
		get_template_part('template-parts/'. $cpt .'box/'. $cpt .'box', $template . '-leftimg' );
		...
		$return =	$startwrapper.
						...
						$footerwrapperend.
					$closewrapper;
	} elseif(!empty($boxview) && $boxview == 'slickview-bottomimg' && $cpt == 'testimonial') {
		$template 			= 'slickview';
		$column				= 'one';
		$startwrapper 		= thememount_column_div('start', $column ).'<div class="testimonial_wrapper">';
		$closewrapper 		= '</div>'.thememount_column_div('end', $column );
		...
		while ( $posts->have_posts() ) {
			...
			//content
			ob_start();
			get_template_part('template-parts/'. $cpt .'box/'. $cpt .'box', $template . '-bottomimg-top' );
			$content .= ob_get_contents();
			ob_end_clean();
			
			// image and title
			ob_start();
			get_template_part('template-parts/'. $cpt .'box/'. $cpt .'box', $template . '-bottomimg-bottom' );
		...
		while ( $posts->have_posts() ) {
			...
			if( !empty($vars['sortable']) && $vars['sortable']=='yes' ){
				$post_terms = wp_get_post_terms( get_the_ID(), thememount_get_taxonomy_from_cpt() );
			...
			if( $cpt == 'client' ){
				...
				if( $add_link == 'yes' && thememount_client_single_link()!='' ){
					$client_wrap_start .= '<a class="tm-client-logo-link" href="' . thememount_client_single_link() . '" target="_blank">';
			...
			$return .= thememount_column_div('start', $column );
				$return .= $client_wrap_start;
					$return .= $boxes;
				$return .= $client_wrap_end;
			$return .= thememount_column_div('end', $column );
	...
	if( !empty($return) ){
		...
		if( isset($vars['pagination']) && $vars['pagination']=='yes' && $vars['boxview']!='carousel' ){
			$return .= thememount_pagination( $posts );
		}
...
/**
 *  ThemeMount Box class function
 */
if( !function_exists('thememount_box_class') ){
function thememount_box_class( $extra_class='' ){
	$return = '';
	
	// getting taxonomy
	$taxonomy = thememount_get_taxonomy_from_cpt();
	...
	if( !empty($extra_class) ){
		$return .= ' ' . thememount_sanitize_html_classes($extra_class);
	}
 ...
 *  ThemeMount Box class function
 */
if( !function_exists('thememount_get_taxonomy_from_cpt') ){
function thememount_get_taxonomy_from_cpt(){
	$return = 'category';
 ...
 */
if( !function_exists('thememount_global_portfolio_template_list') ){
function thememount_global_portfolio_template_list( $for_vc=false ){
	$return = array(
		'overlay'    => esc_attr__('Content overlay on image view', 'fixology'),
		'top-image'  => esc_attr__('Top Image and Bottom Content view', 'fixology'),
	);
 ...
 */
if( !function_exists('thememount_global_blog_template_list') ){
function thememount_global_blog_template_list( $for_vc=false ){
	$return = array(
		"top-image"       => esc_html__("Top image and bottom content (default)", "fixology"),
		"content-overlay" => esc_html__('Content overlay on image', "fixology"),
	);
 ...
 */
if( !function_exists('thememount_global_team_member_template_list') ){
function thememount_global_team_member_template_list( $for_vc=false ){
	$return = array(
		"overlay"    => esc_html__("Overlay content on Image (Default)", "fixology"),
		"topimage-bottomcontent" => esc_html__("Top Image and Bottom Content", "fixology"),
	);
 ...
 */
if( !function_exists('thememount_global_client_template_list') ){
function thememount_global_client_template_list( $for_vc=false ){
	$return = array(
		'simple-logo'    => esc_attr__('Simple Logo view', 'fixology'),
		'separator-logo' => esc_attr__('Logo with Separator view', 'fixology'),
		'boxed-logo'     => esc_attr__('Boxed view', 'fixology'),
	);
 ...
 */
if( !function_exists('thememount_client_single_link') ){
function thememount_client_single_link( $for_vc=false ){
	$return = '';
	if( get_the_ID() ){
		$post_meta = get_post_meta( get_the_ID(), 'thememount_clients_details' , true );
...
if( !function_exists('thememount_column_div') ){
function thememount_column_div($type='start', $column='three' ){
 ...
 *
 * Create your own thememount_box_title() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 * @return void
 */
if ( !function_exists( 'thememount_box_title' ) ){
function thememount_box_title() {
	$return = '';
	
	if( 'link' != get_post_format() && 'quote' != get_post_format() ){
		$return = '<div class="thememount-box-title"><h4><a href="' . get_permalink() . '">' . get_the_title() . '</a></h4></div>';
	}
...
/**
 * Print blog description for box classic view
 *
 * Create your own thememount_blog_description() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 * @return void
 */
if ( !function_exists( 'thememount_blog_description' ) ){
function thememount_blog_description($limit=''){
	
	$blog_text_limit = thememount_get_option('blog_text_limit');
	$return = '';
	...
	if( has_excerpt() ){
		...
		$return  = nl2br( tm_get_short_desc() );
	} else {
		global $more;
		$more = 0;
		$return = strip_shortcodes( get_the_content( '' ) );
	}
 ...
 *
 * Create your own thememount_blogbox_description() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 * @return void
 */
if ( !function_exists( 'thememount_blogbox_description' ) ){
function thememount_blogbox_description($limit=''){
	
	$blog_text_limit = thememount_get_option('blogbox_text_limit');
	$return = '';
	...
	if( has_excerpt() ){
		...
		$return  = nl2br( tm_get_short_desc('box') );
	} else {
		global $more;
		$more = 0;
...
if ( !function_exists( 'tm_get_short_desc' ) ){
function tm_get_short_desc( $for='' ){
	$blog_text_limit = ($for=='box') ? thememount_get_option('blogbox_text_limit') : thememount_get_option('blog_text_limit') ; 
 ...
 *
 * Create your own thememount_blogbox_readmore_text() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 * @return void
 */
if ( !function_exists( 'thememount_blogbox_readmore_text' ) ){
function thememount_blogbox_readmore_text(){
	$blog_readmore_text = thememount_get_option('blog_readmore_text');
	$return             = esc_html__('Read More', 'fixology');
 ...
 *
 * Create your own thememount_blogbox_readmore() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 * @return void
 */
if ( !function_exists( 'thememount_blogbox_readmore' ) ){
function thememount_blogbox_readmore(){
	
	$return        = '';
	$readMore_text = thememount_blogbox_readmore_text();  // Read More word
	
	if( strpos(get_the_content(), '"more-link"')!==false && get_post_format()!='quote' && get_post_format()!='link' ) {
		$return .= '<div class="thememount-blogbox-footer-readmore"><div class="thememount-blogbox-footer-left thememount-wrap-cell">';
...
function thememount_entry_icon() {
	$iconCode = '';
	...
	$iconCode .= '<i class="tm-fixology-icon-'.$icon.'"></i>';
...
/*
 *  ThemeMount Box Wrapper
 */
if( !function_exists('thememount_box_wrapper') ){
function thememount_box_wrapper( $position='start', $cptname='blog', $vars=array() ){
	...
	if( $position=='start' ){
		...
		// Data tags
		$datatags = tm_carousel_data_html( $vars );
		
		$classArray[] = 'thememount-boxes';
		$classArray[] = 'thememount-boxes-'.$cptname;
		$classArray[] = 'thememount-boxes-view-'.$boxview;
		$classArray[] = 'thememount-boxes-col-'. $column;
		$classArray[] = 'thememount-boxes-sortable-'. $sortable;
		$classArray[] = 'thememount-boxes-textalign-'. $txt_align;
		if( !empty($box_spacing) ){ $classArray[] = 'thememount-boxes-spacing-'. $box_spacing; }
		...
		if ( ! empty( $vars['css_animation'] ) ) {
			$classArray[] = tm_getCSSAnimation( $vars['css_animation'] );
		}
		// Extra class
		...
		if( !empty($vars['css']) ){
			$classArray[] = tm_vc_shortcode_custom_css_class($vars['css']);
		}
		
		$return = '<div class="'. implode(' ',$classArray) .'"'. $datatags .'>
		<div class="thememount-boxes-inner thememount-boxes-'. $cptname .'-inner ">';
		
		
	} else {
		
		$return = '</div><!-- .thememount-boxes-inner -->   </div><!-- .thememount-boxes -->  ';
...
if( !function_exists('thememount_get_query_args') ){
function thememount_get_query_args( $cptname='blog', $vars=array() ){
 ...
 *
 * Create your own thememount_blogbox_show_meta() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 */
if ( !function_exists( 'thememount_blogbox_show_meta' ) ){
function thememount_blogbox_show_meta() {
	return true;
}
}
 ...
 * Blogbox meta boxes
 *
 * Create your own thememount_blogbox_single_meta() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 */
if ( !function_exists( 'thememount_blogbox_single_meta' ) ){
function thememount_blogbox_single_meta( $option='date', $args=array() ) {
	$return      = '';
	$icon        = '';
	$icon_prefix = 'tm-fixology-icon';
	...
	switch($option){
		...
		case 'category' :
			...
			$categories_list = get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'fixology' ) );
			if ( !empty($categories_list) ) {
				$return = sprintf( '<span class="thememount-category-links">%2$s</span>',
					$categories_list
				);
			}
			break;
		...
		case 'tags' :
			...
			$tags_list = get_the_tag_list( '', _x( ', ', 'Used between list items, there is a space after the comma.', 'fixology' ) );
			if ( $tags_list ) {
				$return = sprintf( '<span class="thememount-tags-links">%2$s</span>',
					$tags_list
				);
			}
			break;
	...
	// now preparing the output
	if( $return!='' ){
		$return = '<span class="thememount-blogbox-meta-row thememount-blogbox-meta-row-'. $option .'">'. $icon .' '. $return .'</span>';
	}
...
if( !function_exists('thememount_titlebar_classes') ){
function thememount_titlebar_classes(){
	$titlebar_bg_color    = thememount_get_option('titlebar_bg_color');
	$titlebar_text_color  = thememount_get_option('titlebar_text_color');
	$titlebar_view        = thememount_get_option('titlebar_view');
	$titlebar_background  = thememount_get_option('titlebar_background');
	$breadcrumb_on_bottom = thememount_get_option('breadcrumb_on_bottom');
	...
	// Single overwrite
	if( $post_id ){
		$single_tbar_options = get_post_meta( $post_id, '_thememount_metabox_group', true );
...
if( !function_exists('thememount_get_framework_raw_option') ){
function thememount_get_framework_raw_option( $element_id ){
	$return = '';
...
if( !function_exists('thememount_titlebar_title') ){
function thememount_titlebar_title(){
	...
	if( is_singular() || is_home() ){  // single page, single post and single cpt
		...
		if( is_home() ){
			...
			$title = esc_attr__( 'Blog', 'fixology' );  // Setting for Titlebar title
		}
		
		$single_tbar_settings = get_post_meta( $pageID, '_thememount_metabox_group', true );
		...
		$adv_tbar_catarc = thememount_get_option('adv_tbar_catarc');
		$adv_tbar_catarc = ( !empty($adv_tbar_catarc) ) ? esc_html($adv_tbar_catarc.' %s') : esc_html__('Category Archives: %s', 'fixology') ;
		$title = sprintf(
			$adv_tbar_catarc,
			'<span class="tm-titlebar-heading tm-tbar-category-title">' . esc_html(single_cat_title( '', false)) . '</span>'  // for WPML
		);
		
	} else if( is_tag() ){ // Tag
		$adv_tbar_tagarc = thememount_get_option('adv_tbar_tagarc');
		$adv_tbar_tagarc = ( !empty( $adv_tbar_tagarc ) ) ? esc_attr($adv_tbar_tagarc.' %s') : esc_attr__('Tag Archives: %s','fixology') ;
		$title = sprintf(
			$adv_tbar_tagarc,
			'<span class="tm-titlebar-heading tm-tbar-tag-title">' . esc_html( single_tag_title( '', false)) . '</span>'  // for WPML
		);
		...
		if( is_tax('tm_team_group') || is_tax('tm_portfolio_category') ){
			$title = '<span class="tm-titlebar-heading tm-tbar-taxonomy-title">' . esc_html($tax->name) . '</span>';
			
		} else {
			$adv_tbar_postclassified = thememount_get_option('adv_tbar_postclassified');
			global $wp_query;
			$adv_tbar_postclassified = ( !empty($adv_tbar_postclassified) ) ? esc_html($adv_tbar_postclassified.' %s') : esc_html__('Posts classified under: %s', 'fixology') ;
		
			$title = sprintf(
				$adv_tbar_postclassified,
				'<span>' . esc_html($tax->name) . '</span>'
			);
		...
		$adv_tbar_authorarc = thememount_get_option('adv_tbar_authorarc');
		$adv_tbar_authorarc = ( !empty( $adv_tbar_authorarc ) ) ? esc_html($adv_tbar_authorarc.' %s') : esc_html__('Author Archives: %s', 'fixology');
		...
		$title    = sprintf( esc_html__( 'Search Results for %s', 'fixology' ), '<span class="tm-titlebar-heading tm-tbar-search-title">' . get_search_query() . '</span>' );
		...
		if( function_exists('tribe_is_month') && tribe_is_month() && !is_tax() ) { // The Main Calendar Page
			...
			global $wp_query;
			$title = esc_html__( 'Events on: ', 'fixology' ). date('F j, Y', strtotime($wp_query->query_vars['eventDate']));
		...
		// BBPress section
			...
			$title = esc_html__( 'Forum', 'fixology' );
		} else if( is_post_type_archive() ){
			$title = post_type_archive_title('', false);
		} else if ( is_day() ){
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'fixology' ), '<span>' . get_the_date() . '</span>' );
		} elseif( is_month() ){
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'fixology' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'fixology' ) ) . '</span>' );
		} elseif( is_year() ){
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'fixology' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'fixology' ) ) . '</span>' );
		} else {
			if( function_exists('is_bbpress') && is_bbpress() ) {
				$title = esc_html__( 'Forum', 'fixology' );
			} else {
				$title = esc_html__( 'Archives', 'fixology' );
			}
		};
...
if( !function_exists('thememount_titlebar_content') ){
function thememount_titlebar_content(){
	
	$titlebar_hide_breadcrumb = thememount_get_option('titlebar_hide_breadcrumb');
	$titlebar_view            = thememount_get_option('titlebar_view');
	...
	// Left content
	$leftContent = thememount_titlebar_title();
	
	// Right content
	if( !empty($titlebar_hide_breadcrumb) && $titlebar_hide_breadcrumb!='yes' ){
		$rightContent = thememount_titlebar_breadcrumb();
	}
	...
	if( $post_id ){
		$single_titlebar = get_post_meta( $post_id, '_thememount_metabox_group', true );
		// View
...
if( !function_exists('thememount_titlebar_show') ){
function thememount_titlebar_show(){
	$return = true;
	...
	if( $post_id ){
		$single_view = get_post_meta( $post_id, '_thememount_metabox_group', true );
 ...
 */
if( !function_exists('thememount_titlebar_inline_style') ){
function thememount_titlebar_inline_style(){
	$css = '';
	...
	if( $post_id ){
		$page_titlebar = get_post_meta( $post_id, '_thememount_metabox_group' ,true );
		...
		if( !empty($page_titlebar['titlebar_bg_custom_options']) && $page_titlebar['titlebar_bg_custom_options']=='custom' ){
			...
			$css .= thememount_get_background_css(
		...
		if( !empty($page_titlebar['titlebar_font_custom_options']) && $page_titlebar['titlebar_font_custom_options']=='custom' ){
			...
			thememount_enqueue_google_fonts(
				array(
					$page_titlebar['titlebar_heading_font'],
					$page_titlebar['titlebar_subheading_font'],
					$page_titlebar['titlebar_breadcrumb_font']
...
add_action( 'wp_enqueue_scripts', 'thememount_titlebar_inline_style', 18 );
...
/**
 *  thememount_enqueue_google_fonts function
 */
if( !function_exists('thememount_enqueue_google_fonts') ){
function thememount_enqueue_google_fonts( $fontsdata ){
	
	foreach( $fontsdata as $font ){
		if( !empty($font['family']) ){
			thememount_footer_google_fonts_array( $font['family'] , $font['variant'] );
		}
	}
...
if( !function_exists('thememount_topbar_show') ){
function thememount_topbar_show(){
	$return = true;
	
	
	$show_topbar = thememount_get_option('show_topbar');
...
if( !function_exists('thememount_topbar_classes') ){
function thememount_topbar_classes(){
	global $tm_inline_css;
	$full_wide_elements = thememount_get_option('full_wide_elements');
	$topbar_bg_color    = thememount_get_option('topbar_bg_color');
	$topbar_text_color  = thememount_get_option('topbar_text_color');
	$layout             = thememount_get_option('layout');
	$return = array();
...
if( !function_exists('thememount_topbar_content') ){
function thememount_topbar_content(){
	$return = $topbar_left_text = $topbar_right_text = '';
	$topbar_left_text  = thememount_get_option('topbar_left_text');
	$topbar_right_text = thememount_get_option('topbar_right_text');
	...
	if( thememount_fbar_show()==true ){
		$topbar_right_text .= '
		<span class="thememount-fbar-btn ' . thememount_sanitize_html_classes(thememount_fbar_btn_classes()) . '">
			<a href="#" class="thememount-fbar-btn-link tm-bgcolor-skincolor">
				' . thememount_fbar_open_icon() . '
				' . thememount_fbar_close_icon() . '
				<span class="tm-hide">' . esc_html__('Open', 'fixology') . '</span>
			</a>
		</span>';
	}
 ...
 */
if( !function_exists('thememount_topbar_inline_style') ){
function thememount_topbar_inline_style(){
	$return                   = '';
	$topbar_bg_color          =  thememount_get_option('topbar_bg_color');
	...
	if( $topbar_bg_color=='custom' ){
		$return .= '.thememount-topbar-wrapper{background-color:'. $topbar_bg_custom_color .';}';
	}
	
	// Text Color CSS code
	if( $topbar_text_color=='custom' ){
		$return .= '.thememount-topbar-wrapper, .thememount-topbar-wrapper a{color:'. $topbar_text_custom_color .';}';
	}
 ...
 *  Main logo
 */
if( !function_exists('thememount_site_logo') ){
function thememount_site_logo( $logo_type = '' ){
	$logoimg_sticky = thememount_get_option('logoimg_sticky');
	$logoseo        = thememount_get_option('logoseo');
	$logotype       = thememount_get_option('logotype');
	$return     = '';
	$stickylogo = '';
	...
	// Logo code
	$logo_html = thememount_logo();
	
	// Logo tag for SEO
	$logotag    = ( $logoseo=='h1homeonly' && !is_front_page() ) ? 'span' : 'h1' ;
	
	// Preparing logo
	$return .= '<div class="headerlogo thememount-logotype-'. sanitize_html_class($logotype) .' tm-stickylogo-'. sanitize_html_class($stickyLogoClass) .'">';
...
if( !function_exists('thememount_header_links') ){
function thememount_header_links(){
	$return         = '';
	$header_search  = thememount_get_option('header_search');
	$wc_header_icon = thememount_get_option('wc-header-icon');
	$show_topbar    = thememount_get_option('show_topbar');
	$header_style   = thememount_get_option('headerstyle');
	...
	// Floating bar icon
	if( thememount_fbar_show()==true && $show_topbar==false ){
		$return .= '
		<span class="thememount-fbar-btn ' . thememount_sanitize_html_classes(thememount_fbar_btn_classes()) . '">
			<a href="#" class="thememount-fbar-btn-link tm-icolor-' . thememount_sanitize_html_classes( thememount_get_option('fbar_icon_color') ) . '">
				' . thememount_fbar_open_icon() . '
				' . thememount_fbar_close_icon() . '
				<span class="tm-hide">' . esc_html__('Open', 'fixology') . '</span>
			</a>
	...
	if( in_array($header_style, array('infostack','infostack-rtl','infostack-overlay','infostack-overlay-rtl' ) ) ){
		$header_btn_url  = thememount_get_option('header_btn_url');
		$header_btn_text = thememount_get_option('header_btn_text');
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="tm-header-icon tm-header-wc-cart-link"><a href="' . wc_get_cart_url() . '"><i class="tm-fixology-icon-shopping-cart"></i><span class="number-cart">' . esc_attr($count) . '</span></a></span>';
	}
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<div class="tm-header-icon tm-header-search-link"><a href="#"><i class="tm-fixology-icon-search"></i></a><div class="tm-search-overlay"><form method="get" class="tm-site-searchform" action=" ' . esc_url( home_url() ) . '"> <div class="w-search-form-h"><div class="w-search-form-row"><div class="w-search-input"><input type="search" class="field searchform-s" name="s" placeholder="' . $search_input . '" /><button type="submit"><span class="tm-fixology-icon-search"></span></button></div></div></div></form></div></div>';
	}
 ...
 *  One page website
 */
if( !function_exists('thememount_one_page_site_js') ){
function thememount_one_page_site_js(){
	$one_page_site = thememount_get_option('one_page_site');
...
function thememount_header_container_class(){
	...
	$class = '';
	$class = thememount_container_class('header');
 ...
 *  Header container classes
 */
if( !function_exists('thememount_header_widget_text_color_class') ){
function thememount_header_widget_text_color_class(){
	$return             = '';
	$widget_text_color  = thememount_get_option('header_widget_text_color');
...
function thememount_footer_container_class( $class = array() ){
	
	$class = '';
	$class = thememount_container_class('footer');
 ...
 */
if( !function_exists('thememount_floatingbar_container_class') ){
function thememount_floatingbar_container_class(){
	
	$class = '';
	$class = thememount_container_class('floatingbar');
 ...
 */
if( !function_exists('thememount_topbar_container_class') ){
function thememount_topbar_container_class(){
	
	$class = '';
	$class = thememount_container_class('topbar');
 ...
 */
if( !function_exists('thememount_contentarea_container_class') ){
function thememount_contentarea_container_class(){
	
	$class = '';
	$class = thememount_container_class('content');
 ...
 */
if( !function_exists('thememount_container_class') ){
function thememount_container_class($section='header'){
 ...
 *  Header main classes
 */
if( !function_exists('thememount_header_class') ){
function thememount_header_class( $extra_class='' ){
	$header_bg_color              = thememount_get_option( 'header_bg_color' );
	$header_responsive_icon_color = thememount_get_option( 'header_responsive_icon_color' );
	$megamenu_override            = thememount_get_option( 'megamenu-override' );
	$sticky_header_bg_color       = thememount_get_option( 'sticky_header_bg_color' );
	$header_menu_position         = thememount_get_option( 'header_menu_position' );
	$header_text_abovemenu		  = thememount_get_option('header_text_abovemenu');
	
	$class				= array();
	$headerstyle 		= thememount_get_headerstyle();
 ...
 *  Header main classes
 */
if( !function_exists('thememount_header_w_class') ){
function thememount_header_w_class( $extra_class='' ){
	$class					= array();
	$header_text_abovemenu	= thememount_get_option('header_text_abovemenu');
	$header_bg_color		= thememount_get_option('header_bg_color');
	$headerstyle 			= thememount_get_headerstyle();
 ...
 */
if( !function_exists('thememount_header_menuarea_height') ){
function thememount_header_menuarea_height(){
	$return                 = '60';
	$header_menuarea_height = thememount_get_option('header_menuarea_height');
 ...
 *  Header main classes
 */
if( !function_exists('thememount_sticky_header_class') ){
function thememount_sticky_header_class(){
	$class         = '';
	$sticky_header = thememount_get_option('sticky_header');
...
function thememount_headerclass(){
	...
	$dropdown_menu_separator    = thememount_get_option('dropdown_menu_separator');
...
function thememount_get_headerstyle(){
	$return      = 'classic';
	$headerstyle = thememount_get_option('headerstyle');
 ...
 *  Header dynamic class for different settings
 */
if ( !function_exists('thememount_header_style_class') ){
function thememount_header_style_class( $echo=false ){
	$return = '';
	
	// Main header class so we can understand the selected header style
	$curr_headerstyle = thememount_get_headerstyle();
	$headerstyle = thememount_get_headerstyle();
	$headerstyle = str_replace('-overlay','', $headerstyle);
	$headerstyle = str_replace('-rtl','', $headerstyle);
	$return     .= ' tm-header-style-'. $headerstyle;
	
	if (strpos( thememount_get_headerstyle(), 'overlay') !== false) {
		$return .= ' tm-header-overlay';
	}
	
	if ( (strpos( thememount_get_headerstyle(), 'rtl') !== false) || ((is_rtl()) && (in_array($curr_headerstyle, array('classic','classic-overlay','classic-box-overlay','infostack','infostack-overlay')))) ) {
 ...
 *  Header inline style
 */
if( !function_exists('thememount_header_menu_class') ){
function thememount_header_menu_class(){
	global $fixology_theme_options;
...
/**
 *  ThemeMount Floating Bar classes
 */
if( !function_exists('thememount_fbar_show') ){
function thememount_fbar_show(){
	$fbar_show = thememount_get_option('fbar_show');
	$return    = false;
 ...
 */
if( !function_exists('thememount_fbar_btn') ){
function thememount_fbar_btn(){
	$return = '<!-- Open/close button -->
			<span class="thememount-fbar-btn ' . thememount_sanitize_html_classes(thememount_fbar_btn_classes()) . '">
				<a href="javascript:void(0)" class="thememount-fbar-btn-link">
					' . thememount_fbar_open_icon() . '
					' . thememount_fbar_close_icon() . '
					<span class="tm-hide">' . esc_html__('Open', 'fixology') . '</span>
				</a>
 ...
 *  Floating Bar button classes
 */
if( !function_exists('thememount_fbar_btn_classes') ){
function thememount_fbar_btn_classes(){
		
	$topbarbgcolor = thememount_get_option('topbarbgcolor');
	$fbar_position = thememount_get_option('fbar-position');
...
/**
 *  ThemeMount Floating Bar close icon
 */
if( !function_exists('thememount_fbar_open_icon') ){
function thememount_fbar_open_icon(){
	$return = '';
	
	$fbar_handler_icon = thememount_get_option('fbar_handler_icon');
	$return = '<span class="tm-fbar-open-icon">' . thememount_create_icon_from_data( $fbar_handler_icon, true ) . '</span>';
 ...
 *  ThemeMount Floating Bar close icon
 */
if( !function_exists('thememount_fbar_close_icon') ){
function thememount_fbar_close_icon(){
	$return = '';
	
	$fbar_handler_icon_close = thememount_get_option('fbar_handler_icon_close');
	$return = '<span class="tm-fbar-close-icon" style="display:none;">' . thememount_create_icon_from_data( $fbar_handler_icon_close, true ) . '</span>';
...
/**
 *  ThemeMount Floating Bar close icon for content area
 */
if( !function_exists('thememount_fbar_close_icon_for_content_area') ){
function thememount_fbar_close_icon_for_content_area(){
	$return = '';
	
	$fbar_handler_icon_close = thememount_get_option('fbar_handler_icon_close');
	$return = thememount_create_icon_from_data( $fbar_handler_icon_close, true );
 ...
 *  ThemeMount Floating Bar classes
 */
if( !function_exists('thememount_fbar_classes') ){
function thememount_fbar_classes(){
	global $fixology_theme_options;
	
	$fbar_background = thememount_get_option('fbar_background');
	$topbarbgcolor   = thememount_get_option('topbarbgcolor');
	...
	foreach( $optionsArray as $option ){
		...
		$current_val = thememount_get_option($option);
 ...
 */
if(!function_exists('thememount_floatingbar_inline_css')){
function thememount_floatingbar_inline_css(){
	...
	// getting options
	$fbar_show                = thememount_get_option('fbar_show');
	$fbar_bg_color            = thememount_get_option('fbar_bg_color');
	$fbar_text_color          = thememount_get_option('fbar_text_color');
	$fbar_text_custom_color   = thememount_get_option('fbar_text_custom_color');
	$fbar_icon_color          = thememount_get_option('fbar_icon_color');
	$fbar_icon_custom_color   = thememount_get_option('fbar_icon_custom_color');
	$fbar_btn_bg_custom_color = thememount_get_option('fbar_btn_bg_custom_color');
	...
	if($fbar_show){
		...
		if( $fbar_bg_color == 'custom' && !empty( $fbar_bg_custom_color['rgba'] ) ){
			$return .= '.thememount-fbar-box-w:after{background-color:'.esc_attr($fbar_bg_custom_color['rgba']).';}';
		}
		
		// Custom Text Color
		if( $fbar_text_color == 'custom' && !empty($fbar_text_custom_color) ){
			$fbar_text_custom_color  = esc_attr($fbar_text_custom_color);
			$inlineStyle			.= 'color: rgba( ' . thememount_hex2rgb($fbar_text_custom_color) . ', 0.7);';
			$inlineStyle_a			.= 'color: rgba( ' . thememount_hex2rgb($fbar_text_custom_color) . ', 1);';
			$inlineStyle_ah			.= 'color: rgba( ' . thememount_hex2rgb($fbar_text_custom_color) . ', 0.7);';
			$inlineStyle_h  		.= 'color: rgba( ' . thememount_hex2rgb($fbar_text_custom_color) . ', 1);';
			$inlineStyle_border  	.= 'border-color: rgba( ' . thememount_hex2rgb($fbar_text_custom_color) . ', 0.7);';
...
/* =============================================================== */
/* --------------------- Footer functions  ----------------------- */
if( !function_exists('thememount_footer_row_class') ){
function thememount_footer_row_class( $row='first' ){
	$class = '';
	global $fixology_theme_options;
	
	// BG color
	if( !empty($fixology_theme_options[$row.'_footer_bg_color']) ){
		$class .= ' tm-bg tm-bgcolor-'.sanitize_html_class($fixology_theme_options[$row.'_footer_bg_color']);
	}
	
	// Text color
	if( !empty($fixology_theme_options[$row.'_footer_text_color']) ){
		$class .= ' tm-textcolor-'.sanitize_html_class($fixology_theme_options[$row.'_footer_text_color']);
	}
	
	// If bg image is there
	if( !empty($fixology_theme_options[$row.'_footer_bg_all']['image']) ){
	...
	// border type
	if( !empty($fixology_theme_options[$row.'_footer_bordertype']) ){
		$class .= ' tm-bordertype-'.sanitize_html_class($fixology_theme_options[$row.'_footer_bordertype']);
	}
	
	// Top border for FooterInfo area only
	if( $row=='bottom' && !empty($fixology_theme_options['bottom_footer_border_color']) ){
		$class .= ' tm-bordercolor-'.sanitize_html_class($fixology_theme_options['bottom_footer_border_color']);
	}
	
	//widget border color
	if( !empty($fixology_theme_options[$row.'_footer_widget_bordercolor']) ){
		$class .= ' tm-widgetbordercolor-'.sanitize_html_class($fixology_theme_options[$row.'_footer_widget_bordercolor']);
	}
 ...
 *  usage: thememount_footer_google_fonts_array('Raleway', '100');
 */
if( !function_exists('thememount_footer_google_fonts_array') ){
function thememount_footer_google_fonts_array( $font_family, $font_weight='normal' ){
...
if( !function_exists('thememount_portfolio_single_image_path') ){
function thememount_portfolio_single_image_path(){
	$image = '';
	if (has_post_thumbnail()){
...
if( !function_exists('thememount_box_team_social_links') ){
function thememount_box_team_social_links(){
	$return = '';
	
	$data   = thememount_get_meta( 'thememount_team_member_social', 'social_icons_list' );
	...
	if( !empty($data) && is_array($data) && count($data)>0 ){
		...
		$all_social = tm_shared_social_list();
		...
		foreach($data as $social ){
			...
			$return .= '<li><a href="'. $social['social_icons_list_link'] .'" target="_blank" data-tooltip="' . $social_name . '"><i class="tm-fixology-icon-'. $social['social_icons_list_icon'] .'"></i><span class="tm-hide">'. $social_name .'</span></a></li>';
		}
...
if( !function_exists('thememount_short_desc') ){
function thememount_short_desc(){
	$return = '';
 ...
 */
if( !function_exists('fixology_addhttp') ){
	function fixology_addhttp($url){
		if (!preg_match("~^(?:f|ht)tps?://~i", $url)){
			$url = "http://" . $url;
		}
		return $url;
 ...
 */
if( !function_exists('thememount_change_heading_order') ){
function thememount_change_heading_order($input_code=''){
 ...
 *
 * Create your own thememount_testimonial_title() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 */
if(!function_exists('thememount_testimonial_title')){
function thememount_testimonial_title(){
	
	$return      		= '';
	$testimonial_meta   = get_post_meta( get_the_id(), 'thememount_testimonials_details', true );
	$clienturl 			= trim($testimonial_meta['clienturl']);
	$designation		= trim($testimonial_meta['designation']);
	
	$return .= ( !empty($clienturl) ) ? '<h3 class="thememount-author-name"><a href="'.esc_url($clienturl).'" target="_blank">'.get_the_title().'</a></h3>' : '<h3 class="thememount-author-name">'.get_the_title().'</h3>' ;
	$return .= ( !empty($designation) ) ? '<span class="thememount-box-footer">'.esc_attr($designation).'</span>' : '';
 ...
 *
 * Create your own thememount_testimonial_featured_image() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 */
if(!function_exists('thememount_testimonial_featured_image')){
function thememount_testimonial_featured_image($size='thumbnail'){
	
	$return = "";
	$featured_image = thememount_featured_image($size);
	
	$return = ( !empty($featured_image) ) ? $featured_image : '<span class="thememount-icon-box"><i class="demo-icon tm-fixology-icon-quote-left"></i></span>';
 ...
 *
 * Create your own thememount_header_button() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 */
if( !function_exists('thememount_header_text') ){
function thememount_header_text(){
	$return		 = '';
	$headerstyle = thememount_get_headerstyle();
	$header_text = thememount_get_option('header_text');
	...
	if( in_array( $headerstyle, $valid_headerstyle ) && !empty( $header_text ) ){
		$header_text = thememount_wp_kses( $header_text );
...
if( !function_exists('thememount_text_abovemenu') ){
function thememount_text_abovemenu(){
	$return		 = '';
	$headerstyle = thememount_get_headerstyle();
	$header_text_abovemenu = thememount_get_option('header_text_abovemenu');
	...
	if( in_array( $headerstyle, $valid_headerstyle ) && !empty( $header_text_abovemenu ) ){
		$header_text_abovemenu = thememount_wp_kses( $header_text_abovemenu );
...
if( !function_exists('thememount_text_belowmenu') ){
function thememount_text_belowmenu(){
	$return		 = '';
	$headerstyle = thememount_get_headerstyle();
	$header_text_belowmenu = thememount_get_option('header_text_belowmenu');
	...
	if( in_array( $headerstyle, $valid_headerstyle ) && !empty( $header_text_belowmenu ) ){
		$header_text_belowmenu = thememount_wp_kses( $header_text_belowmenu );
 ...
 * Client Logo boxes. 
 *
 * Create your own thememount_get_clientboxes() to override in a child theme.
 *
 * @since Fixology 1.0
 *
 */
if( !function_exists('thememount_get_clientboxes') ){
function thememount_get_clientboxes( $vars = array() ){
	$return 			= '';
	$group 				= ( !empty( $vars['group'] ) ) ? $vars['group'] : '' ;
	$column 			= ( !empty( $vars['column'] ) ) ? $vars['column'] : '' ;
	$show 				= ( !empty( $vars['show'] ) ) ? $vars['show'] : '' ;
	$clients 			= thememount_get_option('clients');
	...
	foreach( $list_of_clients as $key => $val ){
		...
		if( !empty( $client_logo ) ){
			$return .= thememount_column_div( 'start', $column );
			$return .= '<a href="'.esc_url( $client_website ).'" '.$linktarget.' data-tooltip="'.esc_html( $client_name ).'" title="'.esc_html( $client_name ).'">';
			$return .= $client_logo;
			$return .= '</a>';
			$return .= thememount_column_div( 'end', $column );
		} else {
 ...
 */
if( !function_exists('thememount_revslider_array') ){
function thememount_revslider_array( $countonly=false ) {
	$sliders = array();
	...
	if( $countonly==true ){
		...
		if( count($sliders)==0 ){
			$sliders[''] = esc_attr__('(No Slider Found)', 'fixology');
		}
		return $sliders;
	}
}
...
if( !function_exists('thememount_layerslider_array') ){
function thememount_layerslider_array( $countonly=false ){
	...
	if ( function_exists('lsSliders') ) {
		...
		if( $countonly == true ){
			...
			if( count($slider_names) == 0 ){
				$slider_names[''] = esc_attr__('(No Slider Found)', 'fixology');
			}
			return $slider_names;
		}
...
if( !function_exists('thememount_header_slider_show') ){
function thememount_header_slider_show(){
	$return  = false;
	$post_id = false;
	...
	if( $post_id ){
		$page_slider = get_post_meta( $post_id, '_thememount_metabox_group', true );
...
function thememount_header_slider(){
	...
	if( $post_id ){
		...
		if( !empty($page_slider['slidertype']) ){
			...
			$wrapper_class = 'thememount-slider-wide';
			// Boxed layout wrapper
			if( !empty($page_slider['slider_width']) && esc_attr($page_slider['slider_width'])=='boxed' ){
				$wrapper_class = 'container thememount-slider-boxed';
			}
			$return = '<div class="'. thememount_sanitize_html_classes($wrapper_class) .'">'.$return.'</div>';
 ...
 */
if( !function_exists('thememount_sidebar_class') ){
function thememount_sidebar_class($for='row'){
	
	$container = 'container';
	if( thememount_get_option('layout')=='fullwide' ){
		$container = thememount_contentarea_container_class();
	}
	...
	if( in_array( esc_attr(thememount_get_sidebar_info()), array('left','right') ) ){
		$return_container    = $container;
		$return_row          = 'row multi-columns-row';
		$return_content_area = 'col-md-9 col-lg-9 col-xs-12';
		
	} else if( in_array( esc_attr(thememount_get_sidebar_info()), array('both','bothleft','bothright') ) ){
		$return_container    = $container;
		$return_row          = 'row multi-columns-row';
 ...
 */
if( !function_exists('thememount_get_sidebar_info') ){
function thememount_get_sidebar_info(){
	
	// Sidebar Class
	$sidebar = esc_attr( thememount_get_option('sidebar_post') ); // Global settings
	...
	if( is_home() || is_page() || is_singular() ){
		...
		if( is_page() ){
			$sidebar = esc_attr( thememount_get_option('sidebar_page') ); // Global settings
		}
		
		// if Team member
		if( is_singular('tm_team_member') ){
			$sidebar = esc_attr( thememount_get_option('sidebar_team_member') ); // Global settings
		}
		
		// if Portfolio
		if( is_singular('tm_portfolio') ){
			$sidebar = esc_attr( thememount_get_option('sidebar_portfolio') ); // Global settings
		}
		
		
		// Getting sidebar value from Single (page/post/singluar)
		if( !empty($page_id) ){
			$single_sidebar = get_post_meta( $page_id, '_thememount_metabox_sidebar', true);
		...
		if( is_singular('tribe_events') ){
			$sidebar_events = thememount_get_option('sidebar_events');
	...
	if( is_tax('tm_portfolio_category') ){
		$sidebar = esc_attr( thememount_get_option('sidebar_portfolio_category') ); // Global settings
	}
	
	// Team Group
	if( is_tax('tm_team_group') ){
		$sidebar = esc_attr( thememount_get_option('sidebar_team_member_group') ); // Global settings
	}
	
	
	// WooCommerce sidebar class
	if( function_exists('is_woocommerce') && is_woocommerce() ) {
		$sidebar_woocommerce = thememount_get_option('sidebar_woocommerce');
		$sidebar = !empty( $sidebar_woocommerce ) ? esc_attr( $sidebar_woocommerce ) : 'right' ;
		
		
		$post_id = get_option( 'woocommerce_shop_page_id' );
		if( !empty($post_id) ){
			$single_sidebar = get_post_meta( $post_id, '_thememount_metabox_sidebar', true);
			if( !empty($single_sidebar['sidebar']) ){
				$sidebar = $single_sidebar['sidebar'];
			}
		}
	...
	if( function_exists('is_bbpress') && is_bbpress() ) {
		$sidebar_bbpress = thememount_get_option('sidebar_bbpress');
	...
	if( is_search() ){
		$sidebar_search = thememount_get_option('sidebar_search');
 ...
 */
if( !function_exists('thememount_single_get_sidebar_value') ){
function thememount_single_get_sidebar_value(){
	
	// Getting global sidebar value
	global $fixology_theme_options;
	
	// Globally the sidebar of POST will be used
	$sidebar = $fixology_theme_options['sidebar_post'];
	...
	if( is_page() || is_singular() ){
		...
		if( !empty($fixology_theme_options['sidebar_'.$cpt]) ){
			$sidebar = $fixology_theme_options['sidebar_'.$cpt];
		}
		
		// Getting single meta for sidebar
		$single_meta = get_post_meta( $single_id, '_thememount_metabox_sidebar', true );
	...
	if( is_search() ){
		$sidebar = $fixology_theme_options['sidebar_search'];
	}
 ...
 */
if( !function_exists('thememount_single_contentarea_class') ){
function thememount_single_contentarea_class(){
	$return = 'col-md-12 col-lg-12 col-xs-12';
	
	if( is_page() || is_singular() ){
		
		$sidebar = thememount_single_get_sidebar_value();
 ...
 */
if( !function_exists('thememount_single_show_sidebar') ){
function thememount_single_show_sidebar( $side='left' ){
	$return = false;
 ...
 *  Left Sidebar
 */
if( !function_exists('thememount_get_left_sidebar') ){
function thememount_get_left_sidebar(){
	if( in_array( esc_attr(thememount_get_sidebar_info()), array('left','bothleft','bothright','both') ) ){
		get_sidebar( 'left' );
	}
}
}
 ...
 *  Right Sidebar
 */
if( !function_exists('thememount_get_right_sidebar') ){
function thememount_get_right_sidebar(){
	if( in_array( esc_attr(thememount_get_sidebar_info()), array('right','bothleft','bothright','both') ) ){
		get_sidebar( 'right' );
	}
}
}
 ...
 *  Show event price
 */
if( !function_exists('thememount_event_price') ){
function thememount_event_price(){
	$return = '';
	if( function_exists('tribe_get_formatted_cost') ){
		$cost = tribe_get_formatted_cost();
		if ( ! empty( $cost ) ){
			$return = thememount_wp_kses('<div class="tribe-events-event-cost"><span> ' . esc_html( tribe_get_formatted_cost() ) . ' </span></div>');
		}
	}
 ...
 */
if( !function_exists('thememount_event_description') ){
function thememount_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'fixology') . ' <i class="kwicon-fa-angle-right"></i>';
	
	if( has_excerpt() ){
		$return  = get_the_excerpt();
		$return .= '<div class="thememount-post-readmore"><a href="'.get_permalink().'">'.$readMore.'</a></div>';
	} else {
		global $more;
		$more = 0;
 ...
 *  Show sidebar of hide sidebar
 */
if( !function_exists('thememount_comment_row_template') ){
function thememount_comment_row_template($comment, $args, $depth){
	if ( 'div' === $args['style'] ) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
    ...
    <<?php echo esc_attr($tag); ?> <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			 <em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'fixology' ); ?></em>
			  <br />
		<?php endif; ?>

		<div class="comment-meta commentmetadata">
			<?php printf( '<cite class="tm-comment-owner fn">%s</cite> <span class="says">' . esc_html__('says:', 'fixology' ) . '</span>', get_comment_author_link() ); ?>
			
			<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
			/* translators: 1: date, 2: time */
			printf( esc_html__( '%1$s at %2$s', 'fixology' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( esc_html__( '(Edit)', 'fixology' ), '  ', '' );
			?>
		</div>
 ...
 */
if( !function_exists('thememount_author_social_links') ){
function thememount_author_social_links(){
	$return = '';
	$all_socials = array();
	
	// fetching all values
	// $all_socials[SOCIAL_CLASS]  => get_the_author_meta( 'INPUT_NAME' );
	// The "INPUT_NAME" is defined in hooks.php in thememount_author_socials() function. You can add more socials in that function.
	...
	foreach( $all_socials as $social_class => $social_link ){
		if( !empty($social_link) ){
			$return .= '<li><a href="'. $social_link .'" target="_blank"><i class="tm-fixology-icon-'. $social_class .'"></i><span class="tm-hide">'. ucwords($social_class) .'</span></a></li>';
		}
	}
 ...
 */
if( !function_exists('thememount_404_icon') ){
function thememount_404_icon(){
	$icon   = thememount_get_option('error404_big_icon');
 ...
 *  Getting 404 heading
 */
if( !function_exists('thememount_404_heading') ){
function thememount_404_heading(){
	$heading = thememount_get_option('error404_big_text');
	$return  = ( !empty($heading) ) ? '<header class="page-header"> <h1 class="page-title">' . esc_html( $heading ) . '</h1> </header><!-- .page-header -->' : '' ;
	return $return;
}
}
 ...
 */
if( !function_exists('thememount_404_description') ){
function thememount_404_description(){
	$description = thememount_get_option('error404_medium_text');
	$return  = ( !empty($description) ) ? '<div class="page-content"> <p>' . esc_html( $description ) . '</p> </div><!-- .page-content -->' : '' ;
	return $return;
}
}
 ...
 */
if( !function_exists('thememount_search_results_content') ){
function thememount_search_results_content(){
	$return  = '';
		...
		switch( $post_type ){
			...
			case 'post' :
				if( $curr_total_post < $total_post ){
					$data_post[] = thememount_recent_posts();
					$curr_total_post++;
				}
				break;
			...
			case 'page' :
				...
				if( $curr_total_page <= $total_page ){
					...
					$data = thememount_enc_data('<a href="' . get_permalink() . '">' . get_the_title() . '</a>');
			...
			case 'tm_portfolio' :
				if( $curr_total_tm_portfolio <= $total_tm_portfolio ){
					$column   = 'three';
					$template = thememount_get_option('pfcat_view');
					...
					$data_tm_portfolio .= thememount_column_div('start', $column );
						$data_tm_portfolio .= $boxes;
					$data_tm_portfolio .= thememount_column_div('end', $column );
			...
			case 'tm_team_member' :
				if( $curr_total_tm_team_member <= $total_tm_team_member ){
					$column   = 'three';
					$template = thememount_get_option('teamcat_view');
					...
					$data_tm_team_member .= thememount_column_div('start', $column );
						$data_tm_team_member .= $boxes;
					$data_tm_team_member .= thememount_column_div('end', $column );
			...
			case 'tribe_events' :
				...
				if( function_exists('tribe_is_month') ){
					if( $curr_total_tribe_events <= $total_tribe_events ){
						...
						$data_tribe_events .= thememount_column_div('start', $column );
							$data_tribe_events .= $boxes;
						$data_tribe_events .= thememount_column_div('end', $column );
	...
	if( empty($_GET['post_type']) ){
		...
		if( !empty($data_page) || !empty($data_page) ){
			...
			if( !empty($data_page) ){
				$return .= '<div class="col-sm-6">' . thememount_search_results_box_title( 'page' ) . $data_page . '</div>';
			}
			if( !empty($data_post) ){
				$return .= '<div class="col-sm-6">' . thememount_search_results_box_title( 'post' ) . $data_post . '</div>';
			}
			$return .= '</div>';
	...
	if( !empty($data_tm_portfolio) ){
		...
		// Getting title
		$page_title = ( empty($_GET['post_type']) ) ? thememount_search_results_box_title( 'tm_portfolio' ) : '' ;
	...
	if( !empty($data_tm_team_member) ){
		...
		$page_title = ( empty($_GET['post_type']) ) ? thememount_search_results_box_title( 'tm_team_member' ) : '' ;
	...
	if( !empty($data_product) && function_exists('is_woocommerce') ){
		...
		$page_title = ( empty($_GET['post_type']) ) ? thememount_search_results_box_title( 'product' ) : '' ;
	...
	if( !empty($data_tribe_events) ){
		...
		$page_title = ( empty($_GET['post_type']) ) ? thememount_search_results_box_title( 'tribe_events' ) : '' ;
 ...
 */
if( !function_exists('thememount_search_results_box_title') ){
function thememount_search_results_box_title( $post_type='post' ){
	$return        = '';
	$singular_name = '';
	
	$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'" class="label label-default"><i class="tm-fixology-icon-angle-left"></i> '.esc_html__('Back to results','fixology').'</a></small>';
	if( empty($_GET['post_type']) ){
		$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'&post_type=' . $post_type . '" class="label label-default">'.esc_html__('View more','fixology').'</a></small>';
	}
...
if( !function_exists('thememount_search_form') ){
function thememount_search_form(){
	$return = '';
	
	
	$cptList = array(
		'any'            => esc_html__('All selections', 'fixology'),
		'page'           => esc_html__('Pages', 'fixology'),
		'post'           => esc_html__('Blog posts', 'fixology'),
	);
	...
	<div class="tm-sresult-form-wrapper">
					...
					<div class="tm-sresult-form-bottom-w">
						...
						<div class="tm-search-main-box clearfix">
                            <div class="tm-search-text"><strong>' . esc_html__('Search in:','fixology') . '</strong></div>
							<div class="tm-search-select-box">
								' . $dropdown . '
                              <div class="tm-sresult-form-sbtbtn-wrapper">
								<input class="tm-sresult-form-sbtbtn" type="submit" value="' . esc_html__('Search now','fixology') . '" />
							  </div>
						  </div>
 ...
 *  Recent Posts widget function
 */
if( !function_exists('thememount_recent_posts') ){
function thememount_recent_posts( $post='' ){
 ...
 *
 */
if( !function_exists('thememount_footer_copyright_right') ){
function thememount_footer_copyright_right(){
	$right  = thememount_get_option('footer_copyright_right');
	$return = '';
 ...
 */
if( !function_exists('thememount_events_calendar_correction') ){
function thememount_events_calendar_correction(){
	global $posts;
	global $post;
	if( !empty($posts[0]->ID) ){
		$post = $posts[0];
 ...
 *  sanitizes them using the 'sanitize_html_class' function.
 */
if( !function_exists('thememount_sanitize_html_classes') ){
function thememount_sanitize_html_classes($classes, $return_format = 'input'){
 ...
 *
 */
if( !function_exists('thememount_wp_kses') ){
function thememount_wp_kses( $string, $allowed_html_type='' ){
...
function thememount_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) ){
		$format_prefix = _x( '%1$s on %2$s', '1: post format name. 2: date', 'fixology' );
	} else {
		$format_prefix = '%2$s';
	}
	...
	$date = '<div class="thememount-post-date-wrapper">';
		$date .= sprintf( '<div class="thememount-entry-date-wrapper"><span class="thememount-entry-date"><time class="entry-date" datetime="%1$s" >%2$s<span class="entry-month entry-year">%3$s<span class="entry-year">%4$s</span></span></time></span></div>',
			get_the_date( 'c' ),
			get_the_date( 'j' ),
			get_the_date( 'M' ),
			get_the_date( 'Y' ),
	...
	if ( $echo ){
		echo thememount_wp_kses($date);
	} else {
		return thememount_wp_kses($date);
	}
}
endif;
...
if ( ! function_exists( 'fixology_blogstyle2_meta' ) ) :

function fixology_blogstyle2_meta( $metafor="blogbox" ) {
...
if ( ! function_exists( 'fixology_posttime_meta' ) ) :
function fixology_posttime_meta( $metafor="blogbox" ) {
...
if ( ! function_exists( 'thememount_box_postcategory' ) ) :
function thememount_box_postcategory( $metafor="blogbox" ) {
...
if( !function_exists('thememount_get_socialbar_links') ){
function thememount_get_socialbar_links( $tooltip_position='top' , $tooltip='yes' ){
	global $fixology_theme_options;
	...
	$return = '';
	if( !empty($fixology_theme_options['social_icons_list']) ){
		foreach( $fixology_theme_options['social_icons_list'] as $socialicon ){
			...
			$return .= '<li class="tm-social-' . $social_id . ' tm-socialbox-i-wrapper"><a class="tm-socialbox-icon-link tm-socialbox-icon-link-' . $social_id . ' ' . sanitize_html_class($class) . '" target="_blank" href="' . $href . '"><span class="frame-hover"></span><i class="tm-fixology-icon-' . $social_id . '"></i><span class="social_name">'.$social_name.'</span></a></li>' . "\n";
		}
	}
	...
	foreach( $socialArray as $key=>$value ){
		...
		if( $key == 'rss' ){
			if( isset($fixology_theme_options['rss']) && $fixology_theme_options['rss']=='1' ){
			...
			if( isset($fixology_theme_options[$key]) && trim($fixology_theme_options[$key])!='' ){
				$return .= '<li class="'.$key.' "><a target="_blank" href="'.esc_url($fixology_theme_options[$key]).'" data-tooltip="'.$value[1].'"><i class="tm-social-icon-'.$value[0].'"></i></a></li>';
			}
		}
	}
...
if( !function_exists('thememount_infostack_header_content') ){
function thememount_infostack_header_content(){
	$return = '';
	$first_content  = thememount_get_option('infostack_first_col_content');
	$second_content = thememount_get_option('infostack_second_col_content');
	$third_content  = thememount_get_option('infostack_third_col_content');
	
	$return .= ( !empty( $first_content ) ) ? '<div class="widget-odd widget-first widget-1 widget header-widget widget_text">'.do_shortcode($first_content).'</div>' : '' ;
	$return .= ( !empty( $second_content ) ) ? '<div class="widget-even widget-2 widget header-widget widget_text">'.do_shortcode($second_content).'</div>' : '' ;
	$return .= ( !empty( $third_content ) ) ? '<div class="widget-odd widget-last widget-3 widget header-widget widget_text">'.do_shortcode($third_content).'</div>' : '' ;
	
	echo thememount_wp_kses($return);
}
}