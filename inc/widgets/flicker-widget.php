...
add_action( 'widgets_init', 'fixology_flicker_widget' );

function fixology_flicker_widget() {
	register_widget( 'fixology_flicker_widget' );
}

class fixology_flicker_widget extends WP_Widget {
	...
	function __construct() {
		$widget_style = array('classname'   => 'fixology_flicker_widget',
							  'description' => esc_html__('Show photos from Flickr.','fixology') );
		...
		$control_styles = array('width'   => 300,
								'height'  => 350,
								'id_base' => 'fixology_flicker_widget');
		...
		parent::__construct(
			'fixology_flicker_widget', // Base ID
			esc_html__('Fixology Flickr Widget', 'fixology'), // Name
			$widget_style // Args
		);
	}
	...
	function widget( $args, $cur_instance ) {
		...
		echo '<div class="fixology_flicker_widget_wrapper">'; ?>
	...
	function form( $cur_instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php echo esc_html__('Title', 'fixology'); ?>:</label>
		...
		<p>
			...
			<input type="text" class="widefat" id="<?php echo esc_attr($this->get_field_id( 'flickrID' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'flickrID' )); ?>" value="<?php echo esc_html($cur_instance['flickrID']); ?>" />
		</p>
		
		<p>
			<label for="<?php echo esc_html($this->get_field_id( 'postcount' )); ?>">Number of photos:</label>
			<select id="<?php echo esc_html($this->get_field_id( 'postcount' )); ?>" name="<?php echo esc_html($this->get_field_name( 'postcount' )); ?>" class="widefat">
				<?php
				$maxList = 10;
				for( $x=1; $x<=$maxList; $x++ ){
					?>
					<option <?php if ( $x == $cur_instance['postcount'] ) echo 'selected="selected"'; ?>> <?php echo esc_html($x); ?> </option>
					<?php
				}
				?>
			</select>		
		...
		<p>
			<label for="<?php echo esc_html($this->get_field_id( 'type' )); ?>">Type (user or group):</label>
		...
		<p>
			<label for="<?php echo esc_html($this->get_field_id( 'display' )); ?>">Show (random or most recent):</label>
			<select id="<?php echo esc_html($this->get_field_id( 'display' )); ?>" name="<?php echo esc_html($this->get_field_name( 'display' )); ?>" class="widefat">
