...
get_header(); ?>
	...
	<section id="primary" class="content-area <?php echo thememount_sanitize_html_classes(thememount_sidebar_class('content-area')); ?>">
		...
		<?php if ( have_posts() ) : ?>
			...
			$view   = ( !empty($fixology_theme_options['teamcat_view']) ) ? $fixology_theme_options['teamcat_view'] : 'top-image' ;
			$column = ( !empty($fixology_theme_options['teamcat_column']) ) ? $fixology_theme_options['teamcat_column'] : 'three' ;
